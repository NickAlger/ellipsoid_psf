...
from test_psfi_py import make_grid_mesh, build_field
...
def test_clamp_spd_stack():
    ...
    cleaned, modified = psfi.clamp_spd(stack, 1e-4)
...
def test_set_moment_fields_validates_sigma():
    ...
    cleaned, modified = psfi.clamp_spd(field_Sigma, 1e-6)
...
def test_validated_field_still_evaluates():
    ...
    cfg = psfi.EvalConfig(frame=psfi.Frame.whitened_affine, scaling=psfi.Scaling.volume_det,
                          num_neighbors=5)
