...
def test_low_rank_struct():
    ...
    with pytest.raises(ValueError):
        psfi.LowRank(U, pseudo_random(4, 3, 2))
...
def test_truncated_svd_against_numpy():
    ...
    assert psfi.truncated_svd(B, 1e-12).rank == 3
    assert rel_err(B, psfi.truncated_svd(B, 1e-12)) <= 1e-12
    assert psfi.truncated_svd(B, 0.0, max_rank=2).rank == 2
    with pytest.raises(ValueError):
        psfi.truncated_svd(B, -1.0)


def test_recompress():
    U0 = pseudo_random(10, 3, 6)
    V0 = pseudo_random(7, 3, 7)
    inflated = psfi.LowRank(np.hstack([U0, U0]), np.hstack([0.5 * V0, 0.5 * V0]))
    F = psfi.recompress(inflated, 1e-12)
    assert F.rank == 3
...
def test_aca_recovers_and_reports():
    ...
    rtol = 1e-10
    result = psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 30, 25, rtol)
    ...
    again = psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 30, 25, rtol)
...
def test_aca_rank_cap_is_reported():
    ...
    result = psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 20, 15, 1e-12,
                      psfi.ACAOptions(max_rank=3))
...
def test_aca_disconnected_support():
    ...
    result = psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 40, 40, 1e-8)
    assert result.converged
    assert result.factors.rank == 4
...
def test_aca_validation():
    ...
    with pytest.raises(ValueError):
        psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 4, 4, -1.0)
    with pytest.raises(ValueError):
        psfi.aca(lambda i: A[i, :], lambda j: A[:, j], 4, 4, 1e-8,
                 psfi.ACAOptions(required_consecutive_successes=0))
    with pytest.raises(ValueError):
        psfi.aca(lambda i: np.zeros(2), lambda j: A[:, j], 4, 4, 1e-8)


def test_randomized_svd():
    A = pseudo_random(25, 3, 17) @ pseudo_random(18, 3, 18).T
    F = psfi.randomized_svd(lambda X: A @ X, lambda Y: A.T @ Y, 25, 18, 3)
    assert F.rank == 3
    ...
    options = psfi.RSVDOptions(power_iterations=2)
    G = psfi.randomized_svd(lambda X: B @ X, lambda Y: B.T @ Y, 30, 22, 8, options)
    best = rel_err(B, psfi.truncated_svd(B, 0.0, max_rank=8))
    assert rel_err(B, G) <= 10.0 * best
    G2 = psfi.randomized_svd(lambda X: B @ X, lambda Y: B.T @ Y, 30, 22, 8, options)
    assert np.array_equal(G.U, G2.U)

    with pytest.raises(ValueError):
        psfi.randomized_svd(lambda X: A @ X, lambda Y: A.T @ Y, 25, 18, 0)
